...
class TaskCheckOffApiView(APIView):
    ...
    def post(self, request):
        ser = TaskCheckOffSerializer(
            instance=Task.objects.get(id=request.data.get("id")), data=request.data
        )
        if ser.is_valid():
            ser.save()
            return Response(status=status.HTTP_202_ACCEPTED)
...
class TimerStatus(APIView):
    ...
    def get(self, request):
        ...
        instance = Timer.objects.filter(is_completed=False, user=request.user).order_by(
            "-start_time"
        )
        ...
        if instance:
            ...
            if timer_status == "paused":
                ...
                end_time = timezone.now() + datetime.timedelta(
                    minutes=time_left_in_seconds(instance) / 60
                )
            ...
            break_instance = Break.objects.filter(
        ...
        )  # - sum([i.end_time - i.start_time for i in timer.paused.all()])
...
class StartTimeView(APIView):
    ...
    def post(self, request):
        ...
        timer_serializer = TimerSerializer(
            user=request.user, data={"start_time": start_time, "user": request.user.id}
        )  # start time
        if not request.data.get("task"):
        ...
        if timer_serializer.is_valid():
            try:
                with transaction.atomic():
                    ...
                    timer.end_time = timer.start_time + datetime.timedelta(
                        minutes=Configuration.objects.first().pomo_time
                    )
                    timer.is_active = (
                        True  # to  show that timer is active and not completed by force
                    )
                    ...
                    time_task.save()
                    return Response(
                        {"end_time": timer.end_time, "id": timer.id},
...
class UpdateTimeView(APIView):
    ...
    def post(self, request):
        ...
        if request.data.get("state"):
            ...
            if state == "pause":
                ...
                if timer.is_paused:
                    ...
                    if paused_instance:
                        ...
                        timer.end_time = timezone.now() + datetime.timedelta(
                            minutes=time_left_in_seconds(instance=timer) / 60
                        )
                        ...
                        return Response({"timer": ser.data}, status=status.HTTP_200_OK)
                    else:
                        return Response(status=status.HTTP_400_BAD_REQUEST)
                else:
                    return Response(status=status.HTTP_403_FORBIDDEN)
            ...
            return Response(
                {"error": "state field not found"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class CreateBreakView(APIView):
    ...
    def post(self, request):
        ...
        if request.data.get("break_type") == "LONG":
            config = Configuration.objects.get(user=request.user)
            instance.end_time = timezone.now() + datetime.timedelta(
                minutes=(config.long_break_time)
            )
        else:
            instance.end_time = timezone.now() + datetime.timedelta(
                minutes=(config.short_break_time)
            )
        # if instance.val():
        instance.save()
...
class BarChartAPIView(APIView):
    ...
    def get(self, request):
        ...
        if mode == "Week":
            timer = (
                Timer.custom.filter(user=self.request.user)
                .filter(
                    Q(start_time__gt=timezone.now() - datetime.timedelta(days=7))
            ...
            for i in range(0, 7):
                ...
                print(
                    "something is wrong",
                    timer.filter(start_time__day=v.day).count(),
                    "-----dc-vd-dv-",
                )
                if d.get(v_string):
                    d[v_string] += 1
            ...
            timer = (
                Timer.custom.filter(user=self.request.user)
                .filter(
                    Q(start_time__gt=timezone.now() - datetime.timedelta(days=30))
                    | Q(start_time__lt=timezone.now())
                )
                .completed()
                .order_by("start_time")
...
class DashboardAPIView(APIView):
    ...
    def get(self, request):
        ...
        t = timezone.now()
        tasktimer = TaskTimer.objects.filter(task__user=request.user).filter(
            Q(timer__start_time__date=t.date(), timer__end_time__lt=t)
            | Q(timer__start_time__date=t.date(), timer__is_completed=True)
        )
        ...
        break_instance = Break.objects.filter(
            user=request.user, start_time__date=t.date(), end_time__lt=t
        )
        ...
        return Response(
            {"data": ser.data, "break": break_ser.data}, status=status.HTTP_200_OK
        )
