...
class TimerAllStateTestCase(APITestCase):
    def setUp(self):
    ...
    #     self.assertEqual(task_request.status_code,200)
    # def test_post_request(self):
    #     login = self.factory.login(username='testuser',password='testpassword')
    #     # One task
    #     login = self.factory.post('/dj-rest-auth/login/',{'username':'testuser','password':'testpassword'})
    #     print('jwt-auth',self.factory.cookies.keys())
    #     self.assertTrue('jwt-auth' in list(self.factory.cookies.keys()))
    #     # print('start',login.data['access'],'timerallstates')
